...
Goal: minimal schema for Phase 1–3. snake_case, UUID PKs, `created_at`/`updated_at` on every table, all scoped by `org_id` for future multi-tenancy. **The canonical, runnable schema is [`app/supabase/schema.sql`](../app/supabase/schema.sql)** — apply it via the Supabase SQL Editor. Server code reads/writes through the Supabase client (`src/lib/supabase/admin.ts`, secret key); this doc is the conceptual reference.
...
### `companies`
- `id`, `org_id`, `name`, `domain`, `industry`, `employee_count`, `metadata` (jsonb — raw provider/Apollo company blob), `created_at`, `updated_at`
...
### `search_queries` _(planned — not in `schema.sql` yet; NL people-search isn't built)_
- `id`, `org_id`, `user_id`, `nl_query` (text), `compiled_query` (jsonb — what we sent to the search provider), `result_count`, `created_at`
- Useful for auditing + caching + improving the NL→query prompt over time.

## CRM workflow entities

### `deals` _(planned — not in `schema.sql` yet)_
...
### `email_threads`
- `id`, `org_id`, `contact_id`, `subject`, `provider` (`gmail`), `provider_thread_id`, `last_message_at`, `status` (`open` | `replied` | `closed`), `created_at`
...
### `agent_drafts`
- `id`, `org_id`, `contact_id`, `thread_id` (nullable for first-touch), `kind` (`first_touch` | `follow_up` | `reply`), `model` (e.g. `opus-4-8`), `prompt` (text), `body_text` (text), `subject` (text), `status` (`awaiting_approval` | `approved` | `edited` | `rejected` | `sent`), `reviewed_by_user_id` (nullable), `reviewed_at`, `created_at`

### `agent_runs` _(planned — not in `schema.sql` yet; arrives with the Inngest agent layer)_
